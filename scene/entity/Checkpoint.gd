...
func _process(_delta):
	if Global.player1.player_spawnpoint == position + Vector2(0,25):
		$AnimationTree["parameters/Activating_Checkpoint/blend_amount"] = 1
	elif Global.player1.player_spawnpoint != position + Vector2(0,25):
...
func _on_checkpoint_area_body_entered(body):
	if body.name =="Character":
		...
		Global.player1.player_spawnpoint = position + Vector2(0,25)
