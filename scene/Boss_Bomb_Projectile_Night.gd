...
#	connect("boss_createCrystal", Callable(world, "_on_boss_create_crystal"), 4)
	# Normal circle for explosion damage
	$CircleCollision.set_scale(Vector2(0.15, 0.15))
	# Donut for day AOE
	$DonutCollision.set_scale(Vector2(0.15, 0.15))
	$FlameArea.hide()
...
func _process(delta):
	...
	if position == target_position and $DotTick.time_left <= 0:		
		...
		# Change collision size for AOE bombing ONLY and stay as DoT area
		$CircleCollision.set_scale(Vector2(1, 1))
		$DonutCollision.set_scale(Vector2(1, 1))
...
#	print("Change bomb_isExploded to True")
	...
	$CircleCollision.set_disabled(true)
